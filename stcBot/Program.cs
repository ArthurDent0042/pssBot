...
public class IRCbot
{
	...
	public static bool OkToCheckRSS = false;
	public static string rssFeed = string.Empty;
	public static string announceChannel = string.Empty;
	public static BotSettings botSettings;
	public static string torrentHistoryLogFileName;
	public static IConfigurationRoot config = new ConfigurationBuilder()
					.SetBasePath(Directory.GetCurrentDirectory())
					.AddJsonFile(path: "appSettings.json", optional: false, reloadOnChange: true)
					.Build();
	//public static TcpClient client = new TcpClient();
	public static DateTime pingTime = DateTime.Now;
...
static void Main()
	{
		...
		botSettings = new BotSettings()
		{
			...
			RssCheckFrequency = config.GetValue<int>("RSSCheckFrequencyInSeconds")
		};
	...
	{
		System.Timers.Timer timer = new System.Timers.Timer(botSettings.RssCheckFrequency * 1000);
		timer.Start();
	...
	{
		OkToCheckRSS = true;
		if (DateTime.Now - pingTime > TimeSpan.FromMinutes(2))
		{
			// if we haven't heard from the server in 2 minutes, assume we have disconnected
			logger.Error("Disconnected from server - attempting to restart bot (HandleTimerElapsed method)");
			Thread.Sleep(5000);
			IRCbot ircBot = new IRCbot();
			ircBot.Start();
		}
	...
	public void Start()
	{
		...
		using (TcpClient client = new TcpClient())
			try
			{
				...
				client.Connect(botSettings.IrcServer, botSettings.IrcPort);
	...
	public static List<Announce> ReadRSSFeed()
	{
		try
		{
			...
			foreach (SyndicationItem item in feed.Items)
			{
				...
				if (itemSummary.Count() == 4)
				{
					announce.FreeLeech = "Freeleech ";  // the extra space after 'Freeleech' is intentional
				}
